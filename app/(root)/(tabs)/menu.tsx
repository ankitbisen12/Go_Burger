...
const Card = ({
  ...
  return (
    <TouchableOpacity
      ...
      onPress={onPress}
    >
      <Image source={item.image} className="size-16 " resizeMode="cover" />
      <Text
  ...
  return (
    <View className="bg-white rounded-lg shadow-sm flex-row py-4 px-2 mb-4">
      ...
      <View className="flex-1 ml-2">
        ...
        <Text className="text-sm text-gray-600 font-rubik mt-1">
          {item.description}
        </Text>
...
const Menu = () => {
  ...
  const MenuPressHandler = (itemId: string, title: string) => {
    setSelectedMenu(itemId);
    setSelectedMenuField(title);

    console.log("Selected Menu: ", selectedMenuField);
  };
      ...
      <View className="flex flex-row">
        <View className="mr-2 mb-52 border-r border-gray-300">
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <Card
                onPress={() => MenuPressHandler(item.id, item.title)}
                item={item}
