...
export function addToCart(item) {
  return new Promise(async (resolve) => {
    const response = await fetch("http://192.168.181.101:4000/api/v1/cart", {
      method: "POST",
...
export function fetchItemsByUserId() {
  return new Promise(async (resolve) => {
    ...
    console.log("CartApi is running");
    const response = await fetch("http://192.168.181.101:4000/api/v1/cart", {
      headers: {
...
export function updateCart(update) {
  return new Promise(async (resolve) => {
    const response = await fetch(
      "http://192.168.181.101:4000/api/v1/cart/" + update.id,
      {
        method: "PATCH",
...
export function deleteItemFromCart(itemId) {
  ...
  return new Promise(async (resolve) => {
    const response = await fetch(
      "http://192.168.181.101:4000/api/v1/cart/" + itemId,
      {
        method: "DELETE",
        body: JSON.stringify(),
        headers: { "content-type": "application/json" },
